...
namespace Graphs
{
    public partial class PsevdoGraphsForm: Form
    {
        public PsevdoGraphsForm(Substance substance)
        {
            InitializeComponent();
            ...
            added_series_psevdo_2.Color = Color.FromArgb(rand.Next(0, 255), rand.Next(0, 255), rand.Next(0, 255));
            ...
            x_2_sum = 0;
            xy_sum = 0;
            for (int i = 0; i < added_series_psevdo_2.Points.Count; i++)
            {
                y_sum += added_series_psevdo_2.Points[i].YValues[0];
                x_sum += added_series_psevdo_2.Points[i].XValue;
                x_2_sum += added_series_psevdo_2.Points[i].XValue * added_series_psevdo_2.Points[i].XValue;
                xy_sum += added_series_psevdo_2.Points[i].YValues[0] * added_series_psevdo_2.Points[i].XValue;
            }

            y_srd = y_sum / added_series_psevdo_2.Points.Count;
            x_srd = x_sum / added_series_psevdo_2.Points.Count;
            x_2_srd = x_2_sum / added_series_psevdo_2.Points.Count;
            xy_srd = xy_sum / added_series_psevdo_2.Points.Count;

            b = (xy_sum - added_series_psevdo_2.Points.Count * x_srd * y_srd) / (x_2_sum - added_series_psevdo_2.Points.Count * x_srd * x_srd);
            a = y_srd - b * x_srd;
            textBox_Equation_2.Text = Math.Round(a, 5).ToString() + " + " + Math.Round(b, 5).ToString() + " * x";

            Series psevdo_2_trend_line_series = chart_PsevdoGraph_2.Series.Add("Psevdo_2_trendline");
            psevdo_2_trend_line_series.ChartType = SeriesChartType.Line;
            psevdo_2_trend_line_series.Color = added_series_psevdo_2.Color;
            psevdo_2_trend_line_series.BorderWidth = 3;

            x = added_series_psevdo_2.Points[0].XValue;
            y = a + b * x;
            psevdo_2_trend_line_series.Points.AddXY(x, y);
            x = added_series_psevdo_2.Points[added_series_psevdo_2.Points.Count - 1].XValue;
            y = a + b * x;
            psevdo_2_trend_line_series.Points.AddXY(x, y);
            ...
            for (int i = 0; i < added_series_psevdo_2.Points.Count; i++)
            {
                double y_res = a + b * added_series_psevdo_2.Points[i].XValue;
                SS_res += (added_series_psevdo_2.Points[i].YValues[0] - y_res) * (added_series_psevdo_2.Points[i].YValues[0] - y_res);
                SS_reg += (y_res - y_srd) * (y_res - y_srd);
            }
